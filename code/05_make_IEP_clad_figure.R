...
# See this repo for full data/workflow: https://github.com/ryanpeek/DJFMP_zoop
...
load("data/zoop_combined_sf_2014-2018.rda") # zoop_comb_sf
load("data/clad_filt_sf_2014_2018_ams.rda") # clad_crop

# now get spatial info
#load("data/lakes_delta_crop.rda")
load("data/delta_crop_sf.rda")
# simplify
delta_crop <- rmapshaper::ms_simplify(delta_crop, keep = 0.1)
...
ggplot() +
  ...
  # plot clad data
  geom_sf(data=clad_crop, aes(fill=allcladocera_log, size=clad_cut),
          pch=21, color="gray20", alpha=0.9) +
  # plot scale: expression(paste("line1 \n line2 a" [b]))
  scale_fill_viridis_c(name="Mean no. per m^2", option = "A") +
...
#                       guide=guide_colorbar(draw.ulim = F, draw.llim = F)) +
  ...
  theme(axis.text.x = element_blank(),
        ...
        strip.background=element_rect(fill="grey23"),# #5D729D
        strip.text=ggtext::element_textbox(size=11, color = "white", box.color = "grey23"))
